...
% Inlined from canonical experiment JSON for standalone Overleaf compilation.
...
vLLM.  The system intercepts the cache engine without changing the model or
scheduler, binds every custom operation to the current PyTorch CUDA stream,
packs variable-length outputs into persistent slabs, and decodes a restore batch
...
\begin{itemize}
  ...
  workspaces, packed D2H/H2D slabs, current-stream execution, and a batched
...
\node[wide,right=of policy] (compress) {ordered CUDA\\batched encode};
...
A Python monkey patch wraps vLLM V1 cache store and restore operations.  The
model, PagedAttention kernels, scheduler, and block identifiers remain
...
Unsupported layouts or failed compression attempts fall back to the same raw
transfer implementation used by the baseline.

The C++/PyBind11 extension receives the active PyTorch CUDA stream explicitly.
Compression kernels, metadata copies, D2H/H2D copies, and decompression kernels
therefore obey vLLM's existing stream order without a redundant Python-side
device synchronization.  This is essential for both correctness and overlap:
running a fast kernel on the wrong stream either races with a copy or forces a
global barrier.

\subsection{Packed Publication and Restore}

Variable compressed sizes make per-page host allocation and copying expensive.
The GPU-to-CPU path retains completed page payloads in grow-only device
workspaces, computes their offsets, concatenates them into one device slab, and
publishes the batch directly into its final pinned-host slices with one
synchronization.  The host dictionary stores offsets and reconstruction
metadata; it does not copy the payload through pageable Python buffers.
...
We also implement a cost-aware extension that selects raw versus cuSZp, a
model-specific per-layer safety cap, and cuSZp's existing plain, fixed, or
outlier mode.  Offline multi-prompt sweeps conservatively merge per-layer
quality constraints.  Runtime selection uses measured ratio and decoder
throughput, falling back to raw if predicted savings are below a threshold.
Section~\ref{sec:adaptive} reports this mechanism separately: its current
mixed-bound implementation is quality-safe but slower than the fixed path.
...
fixed cuSZp on G2C and 0.707~ms slower on C2G.  Under a continuous synthetic
H2D contender it also remains slower than fixed.  Consequently, adaptive
selection is a working mechanism and a useful overhead ablation, but it is not
part of the paper's primary performance configuration.
\section{Discussion}
...
alone.  Persistent workspaces remove allocation from the per-event critical
path.  Packing converts many small copies and synchronizations into one slab
transfer.  Current-stream execution preserves dependency order without a
device-wide barrier.  Direct BF16 decode eliminates both an FP32 intermediate
and a scatter kernel.  Finally, the matched raw path shows which gains survive
...
needle-retrieval prompts and generates eight tokens.  It does not replace
...
\end{quote}
regenerates vector PDF, 300-dpi PNG, CSV, and LaTeX figure data from the same
canonical JSON.  The experiment runners record the V1-runner environment,
prompt repetitions, cache budget, trial order, arrival interval, SLO
...
with systems work that removes allocation, synchronization, copy fragmentation,
intermediate precision conversion, and scatter overhead.  Five-trial vLLM
